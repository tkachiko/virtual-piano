...
const playKey = event => {
  ...
  const key = document.querySelector(`.piano-key[data-letter="${event.code[3]}"]`);
  if (!key) return;
  if (event.code.length === 4) {
    key.classList.add('piano-key-active-pseudo', 'piano-key-active');
  }
...
const removeActive = () => {
  pianoKeys.forEach(key => key.classList.remove('piano-key-active-pseudo', 'piano-key-active'));
};
